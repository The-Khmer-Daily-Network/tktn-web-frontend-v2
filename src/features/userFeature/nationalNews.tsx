...
export interface NationalNewsCategory {
  id: number;
  name: string;
...
export interface NationalNewsItem {
  id: number;
  ...
  category: NationalNewsCategory | null;
}

// --- Component ---
interface NationalNewsProps {
  initialData?: NationalNewsItem[] | null;
  loading?: boolean;
}

export default function NationalNews({ initialData, loading: loadingProp }: NationalNewsProps = {}) {
  ...
  const getExcerpt = (article: NationalNewsItem) =>
  ...
  if (loading) {
    return (
      <div className="w-full">
        <div className="h-7 bg-gray-200 rounded-xl w-48 mb-4 animate-pulse" />
  ...
  return (
    <div className="w-full">
      <h2
        className={`hidden sm:block text-xl md:text-2xl font-bold text-[#085C9C] mb-4 lg:mb-5 ${getFontClassName("National News")}`}
        style={getFontStyle("National News")}
      >
        National News
      </h2>
      ...
      <div className="sm:hidden mb-6">
        {/* First 5 (keep same carousel style) */}
        {mobileTopFive.length > 0 && (
          <div className="rounded-2xl border border-gray-200 bg-white overflow-hidden mb-4">
            <div className="px-4 pt-5 pb-3">
              <div className="text-center">
                <div
                  className={`text-[18px] font-semibold tracking-[0.18em] text-[#1D2229] ${getFontClassName("NATIONAL NEWS")}`}
                  style={getFontStyle("NATIONAL NEWS")}
                >
                  NATIONAL NEWS
                </div>
        ...
        {/* Remaining 10: first 4 one-column */}
      ...
      <div className="hidden sm:flex flex-col lg:flex-row lg:gap-6 gap-5 mb-6 lg:mb-8">
        ...
        >
          ...
          <div className="mt-3 flex flex-col">
            {/* {featuredArticle.category && (
              <span
                className={`text-xs md:text-sm font-semibold text-[#1D2229] underline decoration-[#085C9C] decoration-2 underline-offset-2 uppercase ${getFontClassName(featuredArticle.category.name)}`}
                style={getFontStyle(featuredArticle.category.name)}
              >
            ...
            <p
              ...
              style={{ color: "rgba(29, 34, 41, 0.65)" }}
            >
        ...
        {/* ——— Small news (sidebar on desktop, below hero on mobile/tablet) ——— */}
              ...
              return (
                ...
                >
                  <div className="flex-1 flex flex-col justify-center min-w-0">
                    {/* {article.category && (
                      <span
                        className={`text-[10px] sm:text-xs font-semibold text-[#1D2229] underline decoration-[#085C9C] decoration-2 underline-offset-1 uppercase ${getFontClassName(article.category.name)}`}
      ...
      {mobileRemainderLastSix.length > 0 && (
        <>
          ...
          {/* Tablet/Desktop: existing row layout */}
